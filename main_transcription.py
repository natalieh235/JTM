try:
    ...
    pass
import torch
### loading dataloader from transcription test!
from dataloader_trans_test import AudioBatchData
from model import CPCEncoder, CPCModel, CPCUnsupersivedCriterion, TranscriptionCriterion, loadModel, getAR, CategoryCriterion
from trainer import run
from datetime import datetime
import os
import argparse
from default_config import setDefaultConfig, rawAudioPath, metadataPathTrain, metadataPathTest, rawLabelsPath
import sys
import random
    ...
    if not config.supervised:
        ...
        if config.task == 'classification':
            ...
            cpcCriterion = TranscriptionCriterion(config.hiddenGar, config.sizeWindow, downsampling, 
                                                  pool=(int(config.transcriptionWindow/10), 0, int(config.transcriptionWindow/10)))
        else:
